...
object Question6 {
  ...
  def main(args: Array[String]) {
    if (args.length < 1) {
      ...
      val sparkConf = new SparkConf().setAppName("Q6WhoContainsMostPopularWord")
	...
	override def compare(a: (String, Int), b: (String, Int)) = {
      ...
      val mostPopularWordTuple = wordCount.max()
      println(mostPopularWordTuple.toString())
      val mostPopularWord = mostPopularWordTuple._1
      
      val num = graph.vertices.filter(x => x._2.contains(mostPopularWord)).count()
      println(s"$num time intervals contains $mostPopularWord")
    }
  }
}
