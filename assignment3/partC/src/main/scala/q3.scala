...
object Question3 {
  ...
  def main(args: Array[String]) {
    if (args.length < 1) {
      ...
      val sparkConf = new SparkConf().setAppName("Q3AverageNumberOfWords")
	...
	(a, b) => (a._1 + b._1, a._2 + b._2)
      ...
      val avgNumber = edgeNumberSum.mapValues(
	(id, value) => value match {
	  case (count, total) => total.toDouble / count
	}
      )
      
      avgNumber.foreach(x => println("(id, avg): " + x))
    }
  }
}
